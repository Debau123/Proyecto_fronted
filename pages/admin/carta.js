// Código restaurado con todos los pasos y lógica original + función de eliminar producto + receta
...
export default function AdminCarta() {
  ...
  const [nuevoProducto, setNuevoProducto] = useState({ nombre: "", descripcion: "", precio: "", imagen: null, categoria: "entrante" });
  ...
  const cargarProductos = () => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/productos?populate=imagen,receta`)
      .then((res) => res.json())
  ...
  const agregarIngredienteUsado = () => {
    if (ingredienteSeleccionado && cantidadSeleccionada) {
      setIngredientesUsados([...ingredientesUsados, { id: parseInt(ingredienteSeleccionado), cantidad: parseFloat(cantidadSeleccionada) }]);
  ...
  const crearTodo = async () => {
    ...
    const uploadRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/upload`, { method: "POST", body: formData });
    ...
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/productos`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        data: {
    ...
    for (const ing of ingredientesUsados) {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/receta-ingredientes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ data: { receta: recetaId, ingrediente: ing.id, cantidad_necesaria: ing.cantidad } }),
      });
    }
  ...
  const eliminarProductoYReceta = async (producto) => {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/productos/${producto.id}`, { method: "DELETE" });
    if (recetaId) await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/recetas/${recetaId}`, { method: "DELETE" });
    cargarProductos();
  };
